...
const sequence = callback => {
  if( isProduction ){
    runSequence('clean:build', 'assemble', 'images', 'sass', 'webpack', 'fonts', 'copy:data', callback);
  } else {
    runSequence('clean:build', 'assemble', 'images', 'sass', 'webpack', 'fonts', 'copy:data', 'sync', 'watch', callback);
  }
};
